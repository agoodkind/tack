...
func RegisterCommands(reg *clispec.Registry, f *cli.Factory) {
	...
	clispec.Register(reg, storeSetRedundancyOp(f))
	clispec.Register(reg, storeSetCoordinatorsOp(f))
	clispec.Register(reg, storeExcludeOp(f))
	clispec.Register(reg, queueStatusOp(f))
	clispec.Register(reg, queueSetReplicationOp(f))
	clispec.Register(reg, queueReplicationProgressOp(f))
	clispec.Register(reg, queueSetMinInsyncOp(f))
	clispec.Register(reg, queueClearThrottleOp(f))
