...
// The replica placement `ops queue set-replication` submits (TACK-409).
...
var (
	...
	// errQueueTooFewBrokers rejects a copy count above the broker count.
	errQueueTooFewBrokers = errors.New("replica count is above the broker count")
	// errQueueDuplicateBroker rejects a broker id listed twice, which would
	// place two copies of one partition on the same broker.
	errQueueDuplicateBroker = errors.New("broker id appears more than once")
)

// PlanReplicaAssignment returns the broker ids for each partition of a topic.
// Partition p takes the requested number of broker ids starting at index p
// modulo the broker count. Leadership then spreads evenly across the brokers,
// and each partition gets distinct brokers.
