...
- [ ] Prevent outstanding old workers from writing into the new index by keeping all their requests bound to the old physical index. Retain the old index for live continuation sessions, then block writes, revoke its write permissions, disable automatic recreation, and delete it through audited cleanup. Rebuild work and cleanup remain resumable.
...
- [ ] Restore a real FDB backup into a disposable local environment using the existing backup/restore operations. Rebuild an empty search index and require current nodes, deleted-node absence, relevance, and final-page text. Never reuse a pre-restore index as authoritative.
