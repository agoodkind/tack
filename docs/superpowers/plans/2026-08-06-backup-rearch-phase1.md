# Backup Rearchitecture Phase 1 (No New Guests) Implementation Plan
...
- Modify: `internal/ops/backup_yb_snapshot_copy.go` (the `yugabyteBackupContainer` constant currently in `backup_yugabyte.go:13` is SHARED with the surviving snapshot-export path, used at :32 and :92; move the constant with its doc comment here unchanged when deleting its old home.)
