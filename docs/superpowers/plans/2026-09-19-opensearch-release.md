...
- [ ] **Verify proposed guest identities against live inventory.** Compare every proposed VMID, IPv6 address, MAC address, Docker subnet, bridge, storage pool, and host assignment with the current Proxmox inventory before saving an OpenTofu plan. Stop on any collision and correct the Configs pull request before continuing.
